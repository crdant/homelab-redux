resource "vsphere_distributed_virtual_switch" "homelab" {
  ...
  host {
    host_system_id = data.vsphere_host.rye.id
    devices = local.homelab_interfaces
  }


  host {
    host_system_id = vsphere_host.bourbon.id
    devices = local.homelab_interfaces
  }
}
...
resource "vsphere_distributed_port_group" "edge-right" {
  name                            = "edge-right-pg"
  distributed_virtual_switch_uuid = "${vsphere_distributed_virtual_switch.homelab.id}"
...
resource "vsphere_distributed_port_group" "edge-left" {
  name                            = "edge-left-pg"
  distributed_virtual_switch_uuid = "${vsphere_distributed_virtual_switch.homelab.id}"
